...
function consoleBlock(lines: string[]) {
    const pre = document.createElement('pre')
    lines.forEach(line => pre.textContent += line + '\n')
    UI.console.appendChild(pre)
}

function consoleData(data: {[key: string]: number}) {
    const lines: string[] = []
    for (let key in data) {
...
function consoleList(values: number[]) {
    const lines: string[] = []
...
function consoleLine(text: string) { consoleBlock([text]) }

function setStatus(text: string) { UI.status.textContent = text }

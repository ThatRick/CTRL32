...
let selectedBlock: UIBlock
function elementSelected(elem: UIBlock, ev: PointerEvent): void {
    ev.stopPropagation()
...
const testWindow = new MovableElement(vec2(100, 100), vec2(200, 200))
    ...
    .setup(elem => {
        elem.append(
            MoveHandle(elem).style({
                width: '100%',
                height: '20px',
                backgroundColor: View.Color.PanelElementHeader
            }).append(
                TextNode('Test Window').padding(2).userSelect('none'),
                Button('X', () => elem.remove())
                    .userSelect('none')
                    .style({
                        position: 'absolute',
                        right: '0px', top: '0px',

...
export class DataTable {
    constructor(data) {
        this.selectionDataType = dataTypes[0];
        this.selectionLittleEndian = true;
        this.columIndex = {
            address: 0,
            value: [...Array(config.bytesPerRow)].map((_, i) => i + 1),
            ascii: [...Array(config.bytesPerRow)].map((_, i) => i + 1 + config.bytesPerRow)
        };
        this.byteValueCells = new Map();
        this.byteAsciiCells = new Map();
        this.modifiedBytes = new Set();
        ...
        switch (ev.key) {
            case 'ArrowLeft':
                if (this.selectionOffset > 0)
                    this.setSelectionOffset(this.selectionOffset - 1);
                ev.preventDefault();
                break;
            case 'ArrowRight':
                if (this.selectionOffset < this.bytes.length - 1)
                    this.setSelectionOffset(this.selectionOffset + 1);
                ev.preventDefault();
                break;
            case 'ArrowUp':
                if (this.selectionOffset >= config.bytesPerRow)
                    this.setSelectionOffset(this.selectionOffset - config.bytesPerRow);
                ev.preventDefault();
                break;
            case 'ArrowDown':
                if (this.selectionOffset < this.bytes.length - config.bytesPerRow)
                    this.setSelectionOffset(this.selectionOffset + config.bytesPerRow);
                ev.preventDefault();
                break;
        }
    ...
    clearSelection() {
        if (this.selectionOffset === undefined)
            return;
        for (let i = this.selectionOffset; i < this.selectionOffset + this.selectionDataType.size; i++) {
    ...
    get element() { return this.table.node; }
}
export class DataViewer {
    constructor(parent) {
        ...
        const headerRoot = htmlElement('div', {
            ...
            setup: (elem) => {
                ...
                elem.ondrop = (ev) => this.openFile(ev);
            }
        });
        this.header = {
            root: headerRoot,
    ...
    openFile(ev) {
        ...
        if (this.title)
            this.title.textContent = config.appName + ' - ' + file.name;

import { HorizontalContainer, VerticalContainer, TextNode, Button, Input } from "../GUI/UIElement.js";
...
export function WebSocketClientView(client) {
    ...
    const Content = VerticalContainer(HorizontalContainer(HostInput, ConnectButton).style({ flexWrap: 'wrap' }), Status, HorizontalContainer(TextNode('Sent:'), SentBytes).style({ justifyContent: 'space-between' }), HorizontalContainer(TextNode('Received:'), ReceivedBytes).style({ justifyContent: 'space-between' }));

...
export function readStruct<T extends StructDefinition>(buffer: ArrayBuffer, startByteOffset: number, struct: T): StructValues<T>
{
...
export function readArrayOfStructs<T extends StructDefinition>(buffer: ArrayBuffer, startByteOffset: number, struct: T, len?: number): Array<StructValues<T>>
{
    let offset = startByteOffset;
...
export function writeStruct<T extends StructDefinition>(buffer: ArrayBuffer, startByteOffset: number, struct: T, values: Partial<StructValues<T>>): number
{
    // console.log('write struct: ', {buffer, startOffset}, struct, values);
    let offset = startByteOffset;
    const view = new DataView(buffer);

    const writeValue = (type: DataType, value: number | undefined = 0) => {
        switch(type) {
...
export function writeStructElement<T extends StructDefinition>(buffer: ArrayBuffer, startByteOffset: number, struct: T, elementName: keyof T, newValue: number) {
    ...
    const updateElement = (type: DataType, value: number | undefined = 0) => {
        switch(type) {
            case DataType.int8:         view.setInt8(offset, value);                offset += DataSize.int8;    break;
            case DataType.uint8:        view.setUint8(offset, value);               offset += DataSize.uint8;   break;
            case DataType.int16:        view.setInt16(offset, value, LE);           offset += DataSize.int16;   break;
...
export function typedArray(buffer: ArrayBuffer, elemType: DataType) {
    switch(elemType) {
        ...
        case DataType.float:    return new Float32Array(buffer);
        case DataType.double:   return new Float64Array(buffer);
    }
}

export function typeName(type: DataType) {
    switch(type) {
...
// Read a struct from buffer
export function readTypedValues(buffer: ArrayBuffer, types: DataType[], startByteOffset = 0): number[]
{
    let offset = startByteOffset;
    const view = new DataView(buffer);
    const values = []
